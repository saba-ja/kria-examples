...
proc set_project_name {} {
    global proj_dict _xil_proj_name_
...
proc create_project_and_set_properties {} {
    global proj_dict _xil_proj_name_ project_dir proj_dir
...
# Function to create source filesets
proc create_src_filesets {{base_folder ""} {target_folder ""}} {
    global proj_dict
    ...
    if {$base_folder eq ""} {
        ...
        set base_dir [file normalize "$base_folder"]
    }
    ...
    if {$target_folder ne ""} {
        set target_src_dir "$base_dir/$src_dir/$target_folder"
    }
...
# Function to set the project ip_repo folder
proc create_ip_repo_path {{base_folder ""}} {
    global proj_dict
...
# Function to create constraint filesets
proc create_constr_filesets {{base_folder ""} {target_folder ""}} {
    global proj_dict
    set origin_dir [file normalize [dict get $proj_dict "proj_addr"]]
    ...
    if {$target_folder ne ""} {
        set target_constr_dir "$base_dir/$constr_dir/$target_folder"
    }
...
# Function to create simulation filesets
proc create_sim_filesets {{base_folder ""} {target_folder ""}} {
    global proj_dict
    ...
    if {$target_folder ne ""} {
        set target_sim_dir "$base_dir/$sim_dir/$target_folder"
    }
...
# Function to create block design and set file properties
proc create_block_design {bd_file design_name validate {base_folder ""} {target_folder ""}} {
    global proj_dict
    ...
    if {$target_folder ne ""} {
        set target_bd_dir "$base_dir/$bd_dir/$target_folder"
    }

    if {[file exists "$target_bd_dir/$bd_file.tcl"]} {
        source -notrace "$target_bd_dir/$bd_file.tcl"
        create_root_design ""
    } else {
        error "Could not source block design file $target_bd_dir/$bd_file.tcl"
    }
...
# Function to create the block design wrapper
proc create_bd_wrapper {bd_name is_top} {
    global proj_dict
...
# Function to set the project top HDL wrapper
proc set_top_wrapper {file_name} {
    # Assume the requested file is already added to 'sources_1' fileset properties
    set obj [get_filesets sources_1]
    set_property -name "top" -value "$file_name" -objects $obj
}

# Function to export block design to tcl
proc cleanup_block_design {bd_name prefix {base_folder ""} {target_folder ""}} {
    global proj_dict
    ...
    set bd_file "$target_bd_dir/$bd_name.tcl"
    set bd_tmp_file "${prefix}_${bd_name}"
...
# Synthesis
# -----------------------------------------------------------------------------
proc set_proj_synthesis { {generic_prop ""}} {
    global proj_dict
...
proc export_proj_bitstream {destination} {
    global proj_dict
    global _xil_proj_name_
    set origin_dir [file normalize [dict get $proj_dict "proj_addr"]]
    set build_folder [dict get $proj_dict "proj_build_dir_name"]
    ...
    file copy -force "$origin_dir/$build_folder/${_xil_proj_name_}/${_xil_proj_name_}.runs/impl_1/${top_module_name}.bit" $destination
    file copy -force "$origin_dir/$build_folder/${_xil_proj_name_}/${_xil_proj_name_}.runs/impl_1/${top_module_name}.ltx" $destination
}
